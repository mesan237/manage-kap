...
import { StatusBar, SectionList, StyleSheet, FlatList, Dimensions } from 'react-native';
...
const _headerData = [...Array(5).keys()].map((i) => ({
  key: faker.string.uuid(), // ✅ correct method
...
const _data = _months.map((month) => ({
  title: month,
  key: month,
  data: [...Array(faker.number.int({ min: 2, max: 12 })).keys()].map(() => ({
...
export default function Analysis() {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        <Animated.View style={[{ padding: _spacing, marginBottom: _spacing }, balanceStylez]}>
          <Text style={[styles.regular, { fontSize: 42, color: _colors.text }]}>
            {faker.finance.amount(500, 8500, 0, '$')}
          </Text>
          <Text style={[styles.regular, { color: _colors.text, opacity: 0.6 }]}>Total Balance</Text>
        </Animated.View>
        <AnimatedFlatList
          data={_headerData}
          ...
          renderItem={({ item, index }) => {
            return (
              ...
              >
                <View style={{ flex: 1 }}>
                  <Text style={[{ color: _colors.text, fontSize: 24 }]}>{item?.emoji}</Text>
                </View>
                <Text style={[styles.bold, { fontSize: 24, color: _colors.text }]}>
                  {item.amount}
                </Text>
                <Text style={[styles.regular, { fontSize: 16, opacity: 0.6, color: _colors.text }]}>
                  {item.department}
                </Text>
              </View>
            );
